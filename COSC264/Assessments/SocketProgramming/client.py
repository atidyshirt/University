...
def start_client(DATE, HOST, PORT, verbose):
    ...
    def decrypt_message(packet):
        ...
        if MagicNo != 0x497E:
            print(
                "MagicNo is incorrect: `{}` recieved, must equal `0x497E`".format(
                    MagicNo
                )
            )
            return -1
        ...
        if PacketType != 0x0002:
            print(
                "PacketType is incorrect: `{}` received, must equal `0x0002`".format(
                    PacketType
                )
            )
        ...
        if LanguageCode < 0x0001 or LanguageCode > 0x0003:
            print(
                "LanguageCode is incorrect: `{}` received, must be within range (1, 3)".format(
                    LanguageCode
                )
            )
            return -1
        ...
        if Month < 1 or Month > 12:
            print(
                "Month is incorrect: `{}` received, must be between 1 and 12".format(
                    Month
                )
            )
            return -1
        ...
        if Day < 1 or Day > 31:
            print(
                "Day is incorrect: `{}` received, must be between 1 and 31".format(Day)
            )
            return -1
        Hour = int.from_bytes(info[10], "big")
        if Hour < 0 or Hour > 23:
            print(
                "Hour is incorrect: `{}` received, must be within range (0, 23)".format(
                    Hour
                )
            )
            return -1
        ...
        if Minute < 0 or Minute > 59:
            print(
                "Minute is incorrect: `{}` received, must be within range (0, 59)".format(
                    Minute
                )
            )
            return -1
    ...
    def format_request(Date):
        ...
        if Date == "date":
            ...
            return -1
        bytelist = [
            MagicNo.to_bytes(2, "big"),
            PacketType.to_bytes(2, "big"),
            RequestType.to_bytes(2, "big"),
        ]
...
def Main():
    ...
    parser.add_argument(
        "MSG",
        help="The message to receive from server must be `date` or `time`",
        type=str,
    )
    ...
    parser.add_argument(
        ...
        "--verbose",
        action="store_true",
        help="verbose output: full output of packet recieved",
    )
